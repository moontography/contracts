...
describe("OKLG NFT", function () {
  //before
  before(async () => {
    const [owner] = await ethers.getSigners();

    //Contract Factory
    OkLetsGoNFTFactory = await ethers.getContractFactory(
      "OKLGNFT"
    );

    //deploy contract, store result in global okLetsGoNFTContract var for tests that follow
    okLetsGoNFTContract =
      await OkLetsGoNFTFactory.deploy(_baseTokenURI);
    ...
    await expect(
      okLetsGoNFTContract
        .connect(owner)
        .setPaymentAddress(testWalletAddress)
    ).to.not.be.reverted;
  });
  ...
  describe("Whitelists", function () {
    ...
    it("Should allow only the owner to add multiple addresses to the presale whitelist", async function () {
      ...
      for (let i = 0; i < numAdditionalForGasTesting; i++) {
        addressesToAddToWhitelist.push(
          new ethers.Wallet.createRandom().address
        );
      }
  ...
  describe("Sales", function () {
    ...
    it("Should allow owner to start/end pre sale", async function () {
      ...
      // Pre sale not active
      expect(await okLetsGoNFTContract.preSaleActive()).to.equal(
        false
      );

      // Public sale not active
      expect(await okLetsGoNFTContract.publicSaleActive()).to.equal(
        false
      );
  ...
  describe("Minting", function () {
    it("Should allow mint of tokens before public sale, only by owner", async function () {
      ...
      expect(
        await okLetsGoNFTContract.balanceOf(signer1.address)
      ).to.be.eq(0);
      ...
      expect(
        await okLetsGoNFTContract.balanceOf(owner.address)
      ).to.be.eq(50);
    });
    ...
    it("Should allow mint of private presale for those on the whitelist only", async function () {
      ...
      expect(
        await okLetsGoNFTContract.balanceOf(signer2.address)
      ).to.be.eq(0);
      ...
      expect(await okLetsGoNFTContract.getMintsLeft()).to.be.eq(
        9949
      );

      //expect token id 51 to exist
      await expect(okLetsGoNFTContract.tokenURI(51)).to.not.be
        .reverted;

      //expect token id 52 to not exist
      await expect(
        okLetsGoNFTContract.tokenURI(52)
    ...
    it("Should not allow minting of more than the allotted maxWalletAmount", async function () {
      ...
      // maxWalletAmount = 5, try minting 6, should revert
      await expect(
        okLetsGoNFTContract
          .connect(signer2)
          .mint(6, {
      ...
      // maxWalletAmount = 5, try minting 2
      await expect(
        okLetsGoNFTContract
          .connect(signer2)
          .mint(2, {
  ...
  describe("Contract Management", function () {
    it("Should allow owner to pause transfers", async function () {
      const [owner, signer1, signer2] = await ethers.getSigners();
      await expect(okLetsGoNFTContract.connect(owner).pause()).to
        .not.be.reverted;
    ...
    it("Should allow owner to unpause transfers", async function () {
      const [owner, signer1, signer2] = await ethers.getSigners();
      await expect(okLetsGoNFTContract.connect(owner).unpause()).to
        .not.be.reverted;
